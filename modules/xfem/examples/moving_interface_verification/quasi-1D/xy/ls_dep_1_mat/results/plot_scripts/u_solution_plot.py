...
for i in range(0,4):
  ...
  moos_04_temp = np.genfromtxt('../u_solutions/nx4/1D_xy_ls1mat_nx04-0.'
   +tstep_num[i]+'.csv',delimiter=',',skip_header=1)
  # Extract XFEM nx=1 solution data
  xfem_01_temp = np.genfromtxt(
    '../u_solutions/nx1/1D_xy_ls1mat_lsf_xfem_nx01-0.'+tstep_num[i]+'.csv',
    delimiter=',',skip_header=1)
  # Extract XFEM nx=4 solution data
  xfem_04_temp = np.genfromtxt(
    '../u_solutions/nx4/1D_xy_ls1mat_lsf_xfem_nx04-0.'+tstep_num[i]+'.csv',
    delimiter=',',skip_header=1)
...
xfem_01_pos = np.array([[0.0,0.94,1.0],
                        [0.0,0.84,1.0],
                        [0.0,0.74,1.0],
                        [0.0,0.64,1.0]])
xfem_04_pos = np.array([[0.0,0.25,0.5,0.75,0.94,1.0],
                        [0.0,0.25,0.5,0.75,0.84,1.0],
                        [0.0,0.25,0.5,0.74,0.75,1.0],
                        [0.0,0.25,0.5,0.64,0.75,1.0]])
...
plt.savefig(savdir+'1D_xy_ls1mat_u_vs_x_05.png')
...
plt.xlabel('Position')
plt.ylabel('Temperature')
plt.axis([0.,1.,390.,640.])
plt.savefig(savdir+'1D_xy_ls1mat_u_vs_x_10.png')
...
plt.xlabel('Position')
plt.ylabel('Temperature')
plt.axis([0.,1.,390.,785.])
plt.savefig(savdir+'1D_xy_ls1mat_u_vs_x_15.png')
...
plt.xlabel('Position')
plt.ylabel('Temperature')
plt.axis([0.,1.,390.,935.])
plt.savefig(savdir+'1D_xy_ls1mat_u_vs_x_20.png')
