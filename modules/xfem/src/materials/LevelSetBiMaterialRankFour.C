  ...
  : LevelSetBiMaterialBase(parameters)
{
  ...
  for (unsigned int i = 0; i < _num_props; i++)
  {
    ...
    _material_props[i] = &declareProperty<RankFourTensor>(_base_name + _prop_names[i]);

    _bimaterial_material_props[i][0] = &getMaterialProperty<RankFourTensor>(
        getParam<std::string>("levelset_positive_base") + "_" + _prop_names[i]);
    _bimaterial_material_props[i][1] = &getMaterialProperty<RankFourTensor>(
