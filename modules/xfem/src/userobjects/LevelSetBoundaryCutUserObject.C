...
#include "LevelSetBoundaryCutUserObject.h"
#include "SubProblem.h"
#include "MooseVariable.h"
#include "libmesh/string_to_enum.h"

registerMooseObject("XFEMApp", LevelSetBoundaryCutUserObject);

template <>
InputParameters
validParams<LevelSetBoundaryCutUserObject>()
{
  InputParameters params = validParams<GeometricCutUserObject>();
  params.addRequiredParam<VariableName>(
      "level_set_var", "The name of the level set variable used to represent the boundary");
  params.addClassDescription("Allows a user to provide a level set one side of "
                             "which will have all elements removed and a boundary "
			     "condition applied along the level set interface.");
  return params;
}

LevelSetBoundaryCutUserObject::LevelSetBoundaryCutUserObject(const InputParameters & parameters)
...
bool
LevelSetBoundaryCutUserObject::cutElementByGeometry(const Elem * elem,
						    ...
						    Real /*time*/) const
{
  ...
  for (unsigned int i = 0; i < n_sides; ++i)
  {
    ...
    if (curr_side->type() != EDGE2)
      mooseError("In LevelSetBoundaryCutUserObject element side must be EDGE2, but type is: ",
						    ...
						    Real /*time*/) const
{
  ...
  for (unsigned int i = 0; i < elem->n_sides(); ++i)
  {
    ...
    if (curr_side->dim() != 2)
      mooseError("In LevelSetBoundaryCutUserObject dimension of side must be 2, but it is ",
                 curr_side->dim());
	...
	mooseError("In LevelSetBoundaryCutUserObject face edge must be EDGE2, but type is: ",
      ...
      dof_id_type ls_dof_id_1 = node1->dof_number(_system.number(), _level_set_var_number, 0);
      dof_id type ls_dof_id_2 = node2->dof_number(_system.number(), _level_set_var_number, 0);
...
bool
LevelSetBoundaryCutUserObject::cutFragmentByGeometry(std::vector<std::vector<Point>> & /*frag_edges*/,
						     std::vector<Xfem::CutEdge> & /*cut_edges*/,
						     Real /*time*/) const
{
  return false;
...
bool
LevelSetBoundaryCutUserObject::cutFragmentByGeometry(std::vector<Point>> & /*frag_faces*/,
						     std::vector<Xfem::CutFace> & /*cut_faces*/,
						     Real /*time*/) const
{
